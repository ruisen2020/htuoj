<template>
  <div class="problemSolveContainer">
    <div v-for="(data, index) in listData" :key="data.articleId">
      <div
        @click="skipAtricle(data.articleId)"
        :style="
          index % 2 === 1
        ...
        <div style="margin-left: 10px; font-size: 14px; line-height: 56px">
        ...
        <div
          v-if="data.problemId != null"
          style="font-size: 14px; line-height: 56px">
          {{ data.problemId }}.{{ data.problemTitle }}
        </div>
        <div style="margin-left: 5px; font-size: 14px; line-height: 56px">
          {{ data.title }}
        </div>
        <div
          style="
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      api.getCollectListByUseId(this.formData).then((response) => {
