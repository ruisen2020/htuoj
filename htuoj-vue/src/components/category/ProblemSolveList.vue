<template>
  <div class="problemSolveContainer">
    <div v-for="(data, index) in listData" :key="data.articleId">
      <div @click="skipAtricle(data.articleId)" :style="index % 2 === 1
        ...
        " style="
          ...
          align-items: center;
        ">
        <div style="margin-left: 20px;line-height: 56px;font-size: 16px;">
          {{ data.problemId }}.{{ data.problemTitle }}
        </div>
        <div style="margin-left: 20px;line-height: 56px;font-size: 16px;">
          {{ data.title }}
        </div>
        <div style="
          ...
          <div class="bottom-info" @click.stop="like(data)">
            <div>
              <span v-if="data.isLike" style="
                  color: #2db55d;
                  display: flex;
                  justify-content: center;
                  align-items: center;
                ">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="1em" height="1em" fill="#2db55d"
          ...
          <div class="bottom-info" @click.stop="collect(data)">
            <div>
              <span v-if="data.isCollect" style="
                  color: #ffa116;
                  display: flex;
                  justify-content: center;
                  align-items: center;
    ...
    <el-pagination @current-change="handleCurrentChange" :current-page.sync="formData.current" background
      :page-size="10" layout="prev, pager, next" :total="total" style="margin-top: 10px">
    </el-pagination>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      api.getArticleListByUserId(this.formData).then((response) => {
