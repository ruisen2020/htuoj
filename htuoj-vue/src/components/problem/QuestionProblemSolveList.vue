<template>
  <div style="overflow: auto; height: 1050px">
    <div v-if="categoryId == 2" style="text-align: right; margin-bottom: 20px">
      <el-button
    ...
    <div v-if="categoryId == 4" style="text-align: right; margin-bottom: 20px">
      <el-button
        style="font-size: 14px"
        icon="el-icon-edit"
        type="primary"
    ...
    <ul
      ...
      class="task-list">
      <li v-for="data in problemSolveList" :key="data.articleId">
            ...
            <div class="blogger-main">
              ...
              <div
                style="
                  ...
                  margin-top: 5px;
                ">
                <span v-if="data.categoryId == 1"> 讨论 |</span>
                <span v-else-if="data.categoryId == 2"> 分享 |</span>
                <span v-else-if="data.categoryId == 3"> 题解 |</span>
                <span v-else-if="data.categoryId == 4"> 公告 |</span>
                {{ data.title }}
              </div>
              ...
              <div
                ...
                <div
                  class="bottom-info"
                  @click.stop="articleInfo(data.articleId)">
                  <div
                    style="
                      display: flex;
                      justify-content: center;
    ...
    <!-- <div
      v-for="data in problemSolveList"
      :key="data.articleId"
      v-infinite-scroll="load"
      infinite-scroll-disabled="disabled">
    ...
    <el-dialog
      :visible.sync="dialogMarkdonwVisible"
      width="1200px"
      ...
      <div style="display: flex">
        <div>
          <el-form
            ...
            <el-form-item
              label=""
              prop="title"
              style="margin-left: 10px; width: 900px">
              <el-input
                style="width: 100%"
                placeholder="请输入标题"
        ...
        <div
          style="margin-left: auto; margin-bottom: 10px; line-height: 40px"
          @click="addArticle">
          <el-button size="small" type="success" round
            ...
            <span v-if="categoryId == 2">发布讨论</span>
            <span v-if="categoryId == 4">发布题解</span>
          </el-button>
        </div>
      </div>
      <div
        style="height: 60%; min-height: 800px; margin-top: 20px"
        id="dialogMarkdown"></div>
...
export default {
  ...
  methods: {
    ...
    load() {
      if (
        this.formData.current * this.formData.size < this.total &&
        !this.loading
      ) {
        this.loading = true;
        setTimeout(() => {
    ...
    getProblemSolveList() {
      api.getArticleListByCategory(this.formData).then((response) => {
        if (response.data.code == "200") {
          this.problemSolveList = this.problemSolveList.concat(
