<template>
  <div class="answering-info">
    ...
    unicode-bidi: isolate; border-radius: 20px">
            <Question
              :count="count"
              :submissionInfoDisplayFlag="submissionInfoDisplayFlag"
              :submissionId="submissionId"
              @submissionId-event="submissionIdEvent"
              @submissionInfoDisplayFlag-event="
                submissionInfoDisplayFlagEvent
              " />
          </el-card>
        </div>
...
export default {
  ...
  components: { Question, CodeMain, TestOutPut },
  data() {
    return {
      loading2: false,
      Result: null,
  ...
  methods: {
    ...
    addSubmission() {
      ...
      this.pollingInterval = setInterval(() => {
        ...
        api
          .getSubmissionById({ submissionId: this.submissionId })
          .then((response) => {
            if (response.data.code === "200") {
